...
import { ArrowLeft, CheckCircle2, Globe2, SkipForward, XCircle } from 'lucide-react';
...
export default function CapitalTrainer() {
  ...
  useEffect(() => inputRef.current?.focus(), [country]);
  ...
  const inputClassName = answerState === 'correct'
    ...
    : answerState === 'incorrect' ? 'border-destructive ring-2 ring-destructive/20' : '';
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 font-display font-bold">
            <Globe2 className="w-5 h-5 text-primary" /> Capital Trainer
          </div>
          <div className="w-20" />
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-10 md:py-16">
        ...
        <section className="rounded-2xl border bg-card p-6 md:p-10 shadow-lg text-center">
          <p className="text-sm font-medium text-primary mb-3">TYPE THE CAPITAL</p>
          <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight">What is the capital of?</h1>
          <div className="mt-8 rounded-xl bg-muted/40 border border-border p-8 md:p-12">
            <img
              src={`https://flagcdn.com/w160/${country.code.toLowerCase()}.png`}
              alt="Country flag"
              className="h-16 w-auto mx-auto rounded shadow-sm mb-5"
            />
            <p className="text-3xl md:text-5xl font-display font-bold">{country.name}</p>
          </div>
